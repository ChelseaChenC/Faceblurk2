...
function screenShot() {
  let imgData;
  saveFrames('out', 'jpg', 1, 1, function(data) {
    imgs = document.getElementsByClassName('screenshot');
    lastShotIndex++;
    lastShotIndex %= imgs.length;
    imgs[lastShotIndex].src = data[0].imageData;
    imgData = data[0].imageData;
    //console.log("EWHATEVER " + data[0].imageData);
    // setTimeout(saveScreen, 3000, imgData);
    setTimeout(saveTest, 100, lastShotIndex);
  });
  // setTimeout(saveScreen, 3000);
}
